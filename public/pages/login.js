...
const Login = () => {
  ...
  const hitLogin = async (obj) => {
    const userResponse = await UserService().login(obj);
    userResponse.status == 401 ? setInvalidCred(true) : router.push('/');
  }

  useEffect(()=>{
    if (typeof window !== 'undefined' && UserService().getCurrentUser()) 
      router.push('/');
    else if(status === "authenticated" && !UserService().getCurrentUser()) {
      if(session['accessToken'] !== undefined){
        ...
        UserService().login(user).then((userResponse)=>{  
          router.push('/')
        });
      }
    }
  ...
  if(status == "authenticated" || status == "loading" || UserService().getCurrentUser()){ 
          ...
          <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
              ...
              <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                  <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                      ...
                      <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit(hitLogin)}>
                          <div>
                      ...
                      <button  onClick={()=>signIn()} className="w-full bg-black rounded-lg p-2 mt-3 text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Sign in with Google</button>
